...
VP_AutoLoader::add_directories(plugin_dir_path( __FILE__ ).'/deeds/vp_new', 'VP_');

// using your own namespace

VP_AutoLoader::add_namespaces('MY_');

VP_AutoLoader::add_directories(plugin_dir_path( __FILE__ ).'/deeds/vp_new/classes', 'MY_');


add_filter( 'vp_field_type_from_class_prefix', 'MY_Class_Prefixes', null, 1 );
if(!class_exists('MY_builder'))

{

    require_once plugin_dir_path( __FILE__ ).'classes/builder.php';
    VP_FileSystem::instance()->resolve_path('includes', 'wpalchemy/MetaBox');
...
$vpfs->add_directories('views'   , WP_PLUGIN_DIR . '/deeds/vp_new/views');
...

{
...

	{
		...
		wp_enqueue_script('vp-metabox', deeds_PLUGIN_URL . 'vafpress/public/js/metabox.min.js', '', '', true);
...

{
    ...
    require_once plugin_dir_path( __FILE__ ).'classes/alchemy.php';
