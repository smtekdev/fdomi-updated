...
if ( ! class_exists( 'Envato_Market_Admin' ) && class_exists( 'Envato_Market' ) ) :
	...
	class Envato_Market_Admin {
		...
		public function __clone() {
			_doing_it_wrong( __FUNCTION__, esc_html__( 'Cheatin&#8217; huh?', 'framework' ), '1.0.0' );
		}
		...
		public function maybe_delete_transients() {
			if ( isset( $_POST[ envato_market()->get_option_name() ] ) ) {
				...
				if ( isset( $_POST['_wpnonce'] ) && ! wp_verify_nonce( $_POST['_wpnonce'], envato_market()->get_slug() . '-options' ) ) {
					wp_die( __( 'You do not have sufficient permissions to delete transients.', 'framework' ) );
				}
		...
		public function add_menu_page() {
			$page = add_menu_page(
				__( 'Envato Market', 'framework' ), __( 'Envato Market', 'framework' ), 'manage_options', envato_market()->get_slug(), array(
					$this,
		...
		public function admin_enqueue_script() {
			...
			$exports = array(
				...
				'i18n'   => array(
					'save'   => __( 'Save', 'framework' ),
					'remove' => __( 'Remove', 'framework' ),
					'cancel' => __( 'Cancel', 'framework' ),
					'error'  => __( 'An unknown error occurred. Try again.', 'framework' ),
				),
			);
		...
		public function render_templates() {
			...
			<script type="text/html" id="tmpl-envato-market-auth-check-button">
				<a href="<?php echo esc_url( add_query_arg( array( 'authorization' => 'check' ), envato_market()->get_page_url() ) ); ?>" class="button button-secondary auth-check-button" style="margin:0 5px"><?php esc_html_e( 'Test API Connection', 'framework' ); ?></a>
			</script>

			<script type="text/html" id="tmpl-envato-market-item">
				<li data-id="{{ data.id }}">
					<span class="item-name"><?php esc_html_e( 'ID', 'framework' ); ?>: {{ data.id }} - {{ data.name }}</span>
					<button class="item-delete dashicons dashicons-dismiss">
						<span class="screen-reader-text"><?php esc_html_e( 'Delete', 'framework' ); ?></span>
					</button>
			...
			<script type="text/html" id="tmpl-envato-market-dialog-remove">
				<div id="envato-market-dialog-remove" title="<?php esc_html_e( 'Remove Item', 'framework' ); ?>">
					<p><?php esc_html_e( 'You are about to remove the connection between the Envato Market API and this item. You cannot undo this action.', 'framework' ); ?></p>
				</div>
			</script>

			<script type="text/html" id="tmpl-envato-market-dialog-form">
				<div id="envato-market-dialog-form" title="<?php esc_html_e( 'Add Item', 'framework' ); ?>">
					<form>
						<fieldset>
							<label for="token"><?php esc_html_e( 'Token', 'framework' ); ?></label>
							<input type="text" name="token" class="widefat" value=""/>
							<p class="description"><?php esc_html_e( 'Enter the Envato API Personal Token.', 'framework' ); ?></p>
							<label for="id"><?php esc_html_e( 'Item ID', 'framework' ); ?></label>
							<input type="text" name="id" class="widefat" value=""/>
							<p class="description"><?php esc_html_e( 'Enter the Envato Item ID.', 'framework' ); ?></p>
						...
						<div class="envato-card-top">
							...
							<div class="column-name">
								<h4>
									...
									<span class="version" aria-label="<?php esc_attr_e( 'Version %s', 'framework' ); ?>"><?php esc_html_e( 'Version', 'framework' ); ?> {{ data.version }}</span>
								</h4>
							</div>
							<div class="column-description">
								...
								<p class="author">
									<cite><?php esc_html_e( 'By', 'framework' ); ?> {{ data.author }}</cite>
								</p>
							</div>
						</div>
						<div class="envato-card-bottom">
							<div class="column-actions">
								<a href="{{{ data.install }}}" class="button button-primary">
									<span aria-hidden="true"><?php esc_html_e( 'Install', 'framework' ); ?></span>
									<span class="screen-reader-text"><?php esc_html_e( 'Install', 'framework' ); ?> {{ data.name }}</span>
								</a>
							</div>
						</div>
					</div>
		...
		public function register_settings() {
			...
			add_settings_section(
				...
				__( 'Activate Envato API Connection', 'framework' ),
			...
			add_settings_section(
				...
				__( 'Single Item Tokens (Advanced)', 'framework' ),
			...
			add_settings_field(
				'items',
				__( 'Envato Market Items', 'framework' ),
		...
		public function install_plugin( $plugin ) {
			if ( ! current_user_can( 'install_plugins' ) ) {
				$msg = '
				<div class="wrap">
					<h1>' . __( 'Installing Plugin...', 'framework' ) . '</h1>
					<p>' . __( 'You do not have sufficient permissions to install plugins on this site.', 'framework' ) . '</p>
					<a href="' . esc_url( 'admin.php?page=' . envato_market()->get_slug() . '&tab=plugins' ) . '">' . __( 'Return to Plugin Installer', 'framework' ) . '</a>
				</div>';
				wp_die( $msg );
			}
			...
			if ( empty( $array_api ) ) {
				...
				<div class="wrap">
					<h1>' . __( 'Installing Plugin...', 'framework' ) . '</h1>
					<p>' . __( 'An error occurred, please check that the item ID is correct.', 'framework' ) . '</p>
					<a href="' . esc_url( 'admin.php?page=' . envato_market()->get_slug() . '&tab=plugins' ) . '">' . __( 'Return to Plugin Installer', 'framework' ) . '</a>
				</div>';
				wp_die( $msg );
			}

			$title              = sprintf( __( 'Installing Plugin: %s', 'framework' ), esc_html( $api->name . ' ' . $api->version ) );
		...
		public function install_theme( $theme ) {
			if ( ! current_user_can( 'install_themes' ) ) {
				$msg = '
				<div class="wrap">
					<h1>' . __( 'Installing Theme...', 'framework' ) . '</h1>
					<p>' . __( 'You do not have sufficient permissions to install themes on this site.', 'framework' ) . '</p>
					<a href="' . esc_url( 'admin.php?page=' . envato_market()->get_slug() . '&tab=themes' ) . '">' . __( 'Return to Theme Installer', 'framework' ) . '</a>
				</div>';
				wp_die( $msg );
			}
			...
			if ( empty( $array_api ) ) {
				...
				<div class="wrap">
					<h1>' . __( 'Installing Theme...', 'framework' ) . '</h1>
					<p>' . __( 'An error occurred, please check that the item ID is correct.', 'framework' ) . '</p>
					<a href="' . esc_url( 'admin.php?page=' . envato_market()->get_slug() . '&tab=themes' ) . '">' . __( 'Return to Plugin Installer', 'framework' ) . '</a>
				</div>';
				wp_die( $msg );
			}
			...
			$title              = sprintf( __( 'Installing Theme: %s', 'framework' ), esc_html( $api->name . ' ' . $api->version ) );
		...
		public function ajax_add_item() {
			if ( ! check_ajax_referer( self::AJAX_ACTION, 'nonce', false ) ) {
				...
				wp_send_json_error( array( 'message' => __( 'The Token is missing.', 'framework' ) ) );
			} elseif ( empty( $_POST['id'] ) ) {
				wp_send_json_error( array( 'message' => __( 'The Item ID is missing.', 'framework' ) ) );
			}
			...
			if ( false === $request ) {
				wp_send_json_error( array( 'message' => __( 'The Token or Item ID is incorrect.', 'framework' ) ) );
			}

			if ( false === envato_market()->api()->download( $_POST['id'], $args ) ) {
				wp_send_json_error( array( 'message' => __( 'The item cannot be downloaded.', 'framework' ) ) );
			}
			...
			wp_send_json_error( array( 'message' => __( 'An unknown error occurred.', 'framework' ) ) );
		}
